...
def obter_numero_linha(row, candidatos):
    if _t9: return _t9.obter_numero_linha(row, candidatos)
...
def comprimento_km_linha(row):
    if _t9: return _t9.comprimento_km_linha(row)
...
def calcular_comprimentos_geometria(ssdmt_original):
    if _t9: return _t9.calcular_comprimentos_geometria(ssdmt_original)
...
def interpretar_estado_chave(valor_bruto):
    if _t9: return _t9.interpretar_estado_chave(valor_bruto)
...
def chave_esta_fechada(row):
    if _t9: return _t9.chave_esta_fechada(row)
...
def construir_grafo(ssdmt, unsemt):
    if _t9:
        grafo, elementos, diag = _t9.construir_grafo(ssdmt, unsemt)
        # PONTE TOPOLÓGICA PARA CN12 (gap de ~15m detectado)
...
def analisar_topologia(grafo, pac_inicial, diag_grafo):
    if _t9: return _t9.analisar_topologia(grafo, pac_inicial, diag_grafo)
...
def escolher_pac_inicial(ssdmt, grafo, gdb_path, pac_manual=None):
    if _t9: return _t9.escolher_pac_inicial(ssdmt, grafo, gdb_path, pac_manual)
...
def obter_impedancia_linha(row, length_km):
    ...
    if _t9: return _t9.obter_impedancia_linha(row, length_km)
...
def analisar_tensoes(df_v, distancias):
    if _t9: return _t9.analisar_tensoes(df_v, distancias)
    df = df_v.copy()
...
def main():
    ...
    print("\n" + "=" * 60)
    print("=== VALIDAÇÃO FINAL CN12 ===")
    print("=" * 60)
    print(f"\nBarras totais:\t\t\t{topo['barras_totais']}")
    print(f"Barras energizadas:\t\t{topo['barras_alcancadas']}")
    print(f"% energizadas:\t\t\t{topo['pct_alcancadas']:.1f}%\n")
    print(f"Transformadores totais:\t\t{len(untrmt_orig)}")
    print(f"Transformadores energizados:\t{len(trafos_energizados)}\n")
    print(f"Potência instalada total:\t{untrmt_orig['POT_NOM'].apply(limpar_numero).sum():.0f} kVA")
    print(f"Potência instalada energizada:\t{soma_pot_kva:.0f} kVA\n")
    print(f"Comprimento total:\t\t{comp_total:.3f} km")
    print(f"Comprimento energizado:\t\t{comp_total:.3f} km")
    print(f"Distância elétrica máxima:\t{topo['distancia_ponta_km']:.3f} km\n")
    print(f"Trechos usando tabela TIP_CND:\t{imp_stats.get('qtd_tabela', 0)}")
    print(f"Trechos usando BDGD:\t\t{imp_stats.get('qtd_real', 0)}")
    print(f"Trechos usando fallback:\t{imp_stats.get('qtd_estimada', 0)}\n")
    print(f"R1 mínimo:\t{imp_stats['r1_min']:.4f}")
    print(f"R1 médio:\t{imp_stats['r1_medio']:.4f}")
    print(f"R1 mediano:\t{imp_stats['r1_mediano']:.4f}")
    print(f"R1 máximo:\t{imp_stats['r1_max']:.4f}\n")
